...
export async function sendContactMessage(
  ...
  if (!names || !email || !message) {
    return fail(
      "Vul in ieder geval jullie namen, e-mailadres en een bericht in.",
    );
  }
  if (!consent) {
    return fail(
      "Zeg nog even JA! tegen de privacyverklaring, dan kan het bericht op pad.",
    );
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    return fail(
      "Dat e-mailadres lijkt niet te kloppen. Kijk er nog even naar.",
    );
  }
  if (date) {
    ...
    if (Number.isNaN(chosen.getTime()) || chosen < today) {
      return fail(
        "De gekozen trouwdatum ligt in het verleden. Kies een datum in de toekomst.",
      );
    }
  }
  if (
  ...
  if (!apiKey) {
    return fail(
      "Het formulier is tijdelijk niet beschikbaar. Mail dan direct naar info@jametanouk.nl.",
    );
  }
  ...
  try {
    ...
    if (error) {
      return fail(
        "Versturen is niet gelukt. Probeer het later opnieuw of mail info@jametanouk.nl.",
      );
    }
    return { ok: true };
  } catch {
